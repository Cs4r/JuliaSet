...
int main(void) {
	...
	// This programs allows to create a julia set only when the number of rows in the image is divisible by the number of processes
	...
	if (process_id == 0) {
		julia_set_image = (char*) malloc(
		                      sizeof(char) * (NUMBER_OF_ROWS * NUMBER_OF_COLUMNS));
	...
	if (process_id == 0) {
		...
		output_file = fopen("./output/parallel1_output.raw", "wb");
		...
		// Write compound image to a file
		for (row = 0; row < NUMBER_OF_ROWS; ++row) {
			fwrite(&(julia_set_image[row * NUMBER_OF_COLUMNS]), sizeof(char), NUMBER_OF_COLUMNS, output_file);
		}
